...
public class InitialAlgebra<Ty, Sym, En, Fk, Att, Gen, Sk, X> 
extends Algebra<Ty, Sym, En, Fk, Att, Gen, Sk, X, Chc<Sk, Pair<X, Att>>>
 implements DP<Ty, Sym, En, Fk, Att, Gen, Sk>  { //is DP for entire instance

	public boolean hasFreeTypeAlgebra() {
	//	return talg().eqs.isEmpty();
		Set<Eq<Ty, Sym, Void, Void, Void, Void, Chc<Sk, Pair<X, Att>>>> l = new HashSet<>();		
		for (Triple<Ctx<Var, Ty>, Term<Ty, Sym, Void, Void, Void, Void, Void>, Term<Ty, Sym, Void, Void, Void, Void, Void>> eq : schema().typeSide.eqs) {
...
/*
	private <Y> Collage<Ty, Sym, Void, Void, Void, Void, Y> addTy(TypeSide<Ty, Sym> ty, Collage<Ty, Sym, Void, Void, Void, Void, Y> talg) {
		Collage<Ty, Sym, Void, Void, Void, Void, Y> ret = new Collage<>(talg);
	...
	private final DP<Ty, Sym, En, Fk, Att, Gen, Sk> dp; //may just be on entity side, if java
	
	private final Map<En, Set<X>> ens;
	private final Map<X, Map<Fk, X>> fks = new HashMap<>();
	private final Map<X, Term<Void, Void, En, Fk, Void, Gen, Void>> reprs = new HashMap<>();	
	private final Map<Term<Void, Void, En, Fk, Void, Gen, Void>, X> nfs = new HashMap<>();
	
	private final Collage<Ty, Sym, En, Fk, Att, Gen, Sk> col;
	...
	public InitialAlgebra(AqlOptions ops, Schema<Ty, Sym, En, Fk, Att> schema, Collage<Ty, Sym, En, Fk, Att, Gen, Sk> col, Iterator<X> fresh, Function<Gen, String> printGen, Function<Sk, String> printSk) {
		this(AqlProver.create(ops, col, schema.typeSide.js), schema, col, fresh, printGen, printSk);
	}
	
	public InitialAlgebra(DP<Ty, Sym, En, Fk, Att, Gen, Sk> dp, Schema<Ty, Sym, En, Fk, Att> schema, Collage<Ty, Sym, En, Fk, Att, Gen, Sk> col, Iterator<X> fresh, Function<Gen, String> printGen, Function<Sk, String> printSk) {
	...
	private boolean add(Term<Void, Void, En, Fk, Void, Gen, Void> term) throws InterruptedException {
	...
	public Term<Void, Void, En, Fk, Void, Gen, Void> repr(X x) {
		...
		Term<Void, Void, En, Fk, Void, Gen, Void> ret = reprs.get(x);
		if (ret == null) {
	...
	private X nf0(Term<Void, Void, En, Fk, Void, Gen, Void> term) {
...
/*
	@Override
	public X nf(Term<Void, Void, En, Fk, Void, Gen, Void> term) {
		X x = nf0(term);
		if (x == null) {
...
*/
	...
	private static <Ty, Sym, En,Fk,Att,Gen,Sk,X,Y> Collage<Ty, Sym, Void, Void, Void, Void, Chc<Sk, Pair<X, Att>>> 
	talg_full(Algebra<Ty, Sym, En, Fk, Att, Gen, Sk, X, Y> alg, Collage<Ty, Sym, En, Fk, Att, Gen, Sk> col) {
		Collage<Ty, Sym, Void, Void, Void, Void, Chc<Sk, Pair<X, Att>>> talg = new Collage<>();
		...
		for (Triple<Pair<Var, En>, Term<Ty, Sym, En, Fk, Att, Void, Void>, Term<Ty, Sym, En, Fk, Att, Void, Void>> eq : alg.schema().eqs) {
			if (alg.schema().type(eq.first, eq.second).left) { //type
				for (X x : alg.en(eq.first.second)) {
					Map<Var, Term<Ty, Sym, En, Fk, Att, Void, Void>> map = new HashMap<>();
					Term<Ty, Sym, En, Fk, Att, Void, Void> q = alg.repr(x).convert(); map.put(eq.first.first, q);
   ...
   //     return dp.eq(ctx, schema.typeSide.js.reduce(lhs), schema.typeSide.js.reduce(rhs)) ; //|| (col.type(ctx, lhs).left ? intoY().equals(intoY(schema.typeSide.js.reduce(rhs))) : false);
	...
	private final List<Pair<Chc<Sk, Pair<X, Att>>, Term<Ty, Sym, Void, Void, Void, Void, Chc<Sk, Pair<X,Att>>>>> list = new LinkedList<>();
	private Term<Ty, Sym, Void, Void, Void, Void, Chc<Sk, Pair<X,Att>>> simpl(Term<Ty, Sym, Void, Void, Void, Void, Chc<Sk, Pair<X,Att>>> term) {
		 //apparently trans can be called before talg()
		for (Pair<Chc<Sk, Pair<X, Att>>, Term<Ty, Sym, Void, Void, Void, Void, Chc<Sk, Pair<X, Att>>>> t : list) {
	...
	private Collage<Ty, Sym, Void, Void, Void, Void, Chc<Sk, Pair<X, Att>>> talg;
	@Override
	public Collage<Ty, Sym, Void, Void, Void, Void, Chc<Sk, Pair<X, Att>>> talg() {
		if (talg != null) {
			return talg;
		}
	...
	public static <En,Ty,Sym,Fk,Att,Gen,Sk,X,Y> Collage<Ty, Sym, Void, Void, Void, Void, Chc<Sk, Pair<X, Att>>> 
	talg(List<Pair<Chc<Sk, Pair<X, Att>>, Term<Ty, Sym, Void, Void, Void, Void, Chc<Sk, Pair<X,Att>>>>> list, Algebra<Ty, Sym, En, Fk, Att, Gen, Sk, X, ?> alg, Collage<Ty, Sym, En,Fk,Att,Gen,Sk> col) {

		List<Eq<Ty, Sym, Void, Void, Void, Void, Chc<Sk, Pair<X, Att>>>> eqs = new LinkedList<>(talg_full(alg, col).eqs);
	//	System.out.println(eqs);
		...
		while (sks_it.hasNext()) {
			...
			Term<Ty, Sym, Void, Void, Void, Void, Chc<Sk, Pair<X, Att>>> replacer = null;
			for (Eq<Ty, Sym, Void, Void, Void, Void, Chc<Sk, Pair<X, Att>>> eq : eqs) {
				if (!eq.ctx.isEmpty()) {
			...
			Term<Ty, Sym, Void, Void, Void, Void, Chc<Sk, Pair<X, Att>>> replacer2 = replacer;
			sks_it.remove();
			
			List<Eq<Ty, Sym, Void, Void, Void, Void, Chc<Sk, Pair<X, Att>>>> eqs2 = new LinkedList<>();
			
			for (Eq<Ty, Sym, Void, Void, Void, Void, Chc<Sk, Pair<X, Att>>> x : eqs) {
		...
		for (Triple<Ctx<Var, Ty>, Term<Ty, Sym, Void, Void, Void, Void, Void>, Term<Ty, Sym, Void, Void, Void, Void, Void>> eq : alg.schema().typeSide.eqs) {
	...
	private Term<Ty, Sym, En, Fk, Att, Gen, Sk> unflatten(Term<Ty, Sym, Void, Void, Void, Void, Chc<Sk, Pair<X, Att>>> term) {
		if (term.obj != null) {
	...
	@Override
	public Term<Ty, Sym, Void, Void, Void, Void, Chc<Sk, Pair<X, Att>>> att(Att att, X x) {
		return reprT0(Chc.inRight(new Pair<>(x, att)));
	}

	
	private Term<Ty, Sym, Void, Void, Void, Void, Chc<Sk, Pair<X, Att>>> reprT0(Chc<Sk, Pair<X, Att>> y) {
        return schema().typeSide.js.java_tys.isEmpty() ? simpl(Term.Sk(y)) : schema.typeSide.js.reduce(simpl(Term.Sk(y)));
	} 
	
	
	@Override
	public Term<Ty, Sym, En, Fk, Att, Gen, Sk> reprT_protected(Term<Ty, Sym, Void, Void, Void, Void, Chc<Sk, Pair<X, Att>>> y) {
        return schema().typeSide.js.java_tys.isEmpty() ? unflatten(simpl(y)) : unflatten(schema.typeSide.js.reduce(simpl(y)));
	} 

	@Override
	public Term<Ty, Sym, Void, Void, Void, Void, Chc<Sk, Pair<X, Att>>> sk(Sk sk) {
		return reprT0(Chc.inLeft(sk));
	}
	
	
	//TODO aql why does using algebra's version cause infinite loop?
	private static <Ty, Sym, Att, Sk, En, Fk, Gen, X, Y> Term<Ty, Sym, Void, Void, Void, Void, Chc<Sk, Pair<X, Att>>> transX(Algebra<Ty, Sym, En, Fk, Att, Gen, Sk, X, Y> alg, Term<Ty, Sym, En, Fk, Att, Gen, Sk> term) {
		if (term.obj != null) {
			return Term.Obj(term.obj, term.ty);
		} else if (term.sym != null) {
	...
	private static <Ty, Sym, En, Fk, Att, Gen, Sk, X, Y> X trans1X(Algebra<Ty, Sym, En, Fk, Att, Gen, Sk, X, Y> alg, Term<Void, Void, En, Fk, Void, Gen, Void> term) {
		if (term.gen != null) {
			return alg.nf(term);
