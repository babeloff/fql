...
class Inferrer {
	...
	private static <Ty, Sym, En, Fk, Att> String inferInstance(Schema<Ty, Sym, En, Fk, Att> a) {
		...
		String ens = "\tgenerators\n" + Util.sep(Util.alphabetical(a.ens).stream().map(z -> "\t\t[list Generators here] : " + z).collect(Collectors.toList()), "\n");
		String eqs = "\tmulti_equations\n" + Util.sep(Util.alphabetical(a.fks.keySet()).stream().map(z -> "\t\t" + z + " -> [list assignments here] // " + a.fks.get(z).first + " -> " + a.fks.get(z).second).collect(Collectors.toList()), "\n");
		String eqs2 = "\t\n" + Util.sep(Util.alphabetical(a.atts.keySet()).stream().map(z -> "\t\t" + z + " -> [list assignments here] // " + a.atts.get(z).first + " -> " + a.atts.get(z).second).collect(Collectors.toList()), "\n");
	...
	private static <Ty, Sym, En, Fk, Att, Ty0, En0, Sym0, Fk0, Att0, Gen, Sk, Gen0, Sk0> String inferTransform(Instance<Ty, Sym, En, Fk, Att, Gen, Sk, ?, ?> a, Instance<Ty0, En0, Sym0, Fk0, Att0, Gen0, Sk0, ?, ?> b) {
		...
		String sks = Util.sep(a.sks().keySet().stream()
				.map(z -> "\t\t" + z + " -> some type side symbols [" + pr(b.schema().typeSide.syms.keySet()) + "] applied to Attributes [" + pr(b.schema().atts.keySet()) + "] applied to paths of foreign keys [" + pr(b.schema().fks.keySet()) + "] applied to Generators [" + pr(b.gens().keySet()) + "] and labelled nulls [" + pr(b.sks().keySet()) + "]  ending at " + a.sks().get(z))
				.collect(Collectors.toList()), "\n");

		String gens = Util.sep(Util.alphabetical(a.gens().keySet()).stream().map(z -> "\t\t" + z + " -> a generator [" + pr(a.gens().keySet()) + "] ." + " a path of foreign keys [" + pr(b.schema().fks.keySet()) + "] ending at " + a.gens().get(z)).collect(Collectors.toList()), "\n");
	...
	private static <Ty, Sym, En, Fk, Att, Ty0, En0, Sym0, Fk0, Att0> String inferMapping(Schema<Ty, Sym, En, Fk, Att> a, Schema<Ty0, En0, Sym0, Fk0, Att0> b) {
		...
		String ens = "\tentities\n" + Util.sep(Util.alphabetical(a.ens).stream().map(z -> "\t\t" + z + " -> an entity [" + pr(b.ens) + "]").collect(Collectors.toList()), "\n");
		String fks = "\tforeign_keys\n" + Util.sep(Util.alphabetical(a.fks.keySet()).stream().map(z -> "\t\t" + z + " -> a path of foreign keys [" + pr(b.fks.keySet()) + "] // " + a.fks.get(z).first + " -> " + a.fks.get(z).second).collect(Collectors.toList()), "\n");
		String atts = "\tattributes\n" + Util.sep(Util.alphabetical(a.atts.keySet()).stream()
				.map(z -> "\t\t" + z + " -> lambda v. some type side symbols [" + pr(b.typeSide.syms.keySet()) + "] applied to Attributes [" + pr(b.atts.keySet()) + "] " + "applied to paths of foreign keys [" + pr(b.fks.keySet()) + "] ending on variable [v] " + " // " + a.atts.get(z).first + " -> " + a.atts.get(z).second + " ").collect(Collectors.toList()), "\n");
		return ens + "\n" + fks + "\n" + atts;
	}
	...
	private static <Ty, Sym, En, Fk, Att, Ty0, En0, Sym0, Fk0, Att0> String inferQuery(Schema<Ty, Sym, En, Fk, Att> a, Schema<Ty0, En0, Sym0, Fk0, Att0> b) {
		...
		@SuppressWarnings("unchecked")
		String ens = "\tentities //source entities: " + pr(a.ens) + "\n" + Util.sep(Util.alphabetical(b.ens).stream().map(z -> "\t\t" + z + " -> " + inferBlock(z, a, b)).collect(Collectors.toList()), "\n");
		String fks = "\tforeign_keys\n" + Util.sep(Util.alphabetical(b.fks.keySet()).stream().map(z -> "\t\t" + z + " [:" + b.fks.get(z).first + " ->" + b.fks.get(z).second + "] -> " + inferTrans(z, a, b)).collect(Collectors.toList()), "\n");
		return ens + "\n\n" + fks;
	...
	private static <Ty, Sym, En, Fk, Att, Ty0, En0, Sym0, Fk0, Att0> String inferTrans(Fk0 fk, Schema<Ty, Sym, En, Fk, Att> a, Schema<Ty0, En0, Sym0, Fk0, Att0> b) {
		...
		String gens = Util.sep(Util.alphabetical(dom).stream().map(z -> z + " -> a generator [" + pr(cod) + "] ." + " a path of foreign keys [" + pr3(a.fks.map) + "]").collect(Collectors.toList()), "\n\t\t\t\t");
		return "{" + gens + "}";
