...
public class ColimitSchema<N> implements Semantics {
	...
	public final Ctx<N, Schema<Ty, Sym, En, Fk, Att>> nodes;
	
	//public final Schema<Ty, Sym, Set<Pair<N,En>>, Pair<N,Fk>, Pair<N,Att>> schema;
	
	//public final Ctx<N, Mapping<Ty, Sym, En,Fk,Att,Set<Pair<N,En>>,Pair<N,Fk>,Pair<N,Att>>> mappings;

	//actually final 
	public final Schema<Ty, Sym, SchExpRaw.En, SchExpRaw.Fk, SchExpRaw.Att> schemaStr;
	
	//actually final
	public final Ctx<N, Mapping<Ty,Sym,En,Fk,Att,En,Fk,Att>> mappingsStr;
	...
	public ColimitSchema<N> renameEntity(En src, En dst, boolean checkJava) {
		...
		for (Triple<Pair<Var, En>, Term<Ty, Sym, En, Fk, Att, Void, Void>, Term<Ty, Sym, En, Fk, Att, Void, Void>> eq : schemaStr.eqs) {
		...
		DP<Ty, Sym, En, Fk, Att, Void, Void> dp = new DP<Ty, Sym, En, Fk, Att, Void, Void>() {
			@Override
			...
			@Override
			//TODO aql check this
			public boolean eq(Ctx<Var, Chc<Ty, En>> ctx, Term<Ty, Sym, En, Fk, Att, Void, Void> lhs, Term<Ty, Sym, En, Fk, Att, Void, Void> rhs) {
		...
		Schema<Ty, Sym, En, Fk, Att> schemaStr2 
		...
		Map<Att, Triple<Var, En, Term<Ty, Sym, En, Fk, Att, Void, Void>>> attsM = new HashMap<>();
		for (Att k : schemaStr.atts.keySet()) {
			attsM.put(k, new Triple<Var, En, Term<Ty, Sym, En, Fk, Att, Void, Void>>(isoToUser.atts.get(k).first, isoToUser.atts.get(k).second.equals(src) ? dst : isoToUser.atts.get(k).second, isoToUser.atts.get(k).third.mapAtt(updateAtt).mapFk(updateFk)));
		}
		...
		Map<Att, Triple<Var, En, Term<Ty, Sym, En, Fk, Att, Void, Void>>> attsM2 = new HashMap<>();
	...
	public ColimitSchema<N> removeFk(Fk src, List<Fk> l, boolean checkJava) {
		Var v = new Var("v");
		Term<Ty, Sym, En, Fk, Att, Void, Void> t = Term.Fks(l, Term.Var(v));
		if (!schemaStr.fks.containsKey(src)) {
		...
		Set<Triple<Pair<Var, En>, Term<Ty, Sym, En, Fk, Att, Void, Void>, Term<Ty, Sym, En, Fk, Att, Void, Void>>> eqs = new HashSet<>();
		for (Triple<Pair<Var, En>, Term<Ty, Sym, En, Fk, Att, Void, Void>, Term<Ty, Sym, En, Fk, Att, Void, Void>> eq : schemaStr.eqs) {
			Triple<Pair<Var, En>, Term<Ty, Sym, En, Fk, Att, Void, Void>, Term<Ty, Sym, En, Fk, Att, Void, Void>> 
	...
	public ColimitSchema<N> removeAtt(Att src, Var v, Term<Ty, Sym, En, Fk, Att, Void, Void> t, boolean checkJava) {
		if (!schemaStr.atts.containsKey(src)) {
		...
		Schema<Ty, Sym, En, Fk, Att> schemaStr2 
		= new Schema<>(ty, schemaStr.ens, atts, schemaStr.fks.map, eqs, dp, checkJava);  
		Map<Att, Triple<Var, En, Term<Ty, Sym, En, Fk, Att, Void, Void>>> attsM = new HashMap<>(isoToUser.atts.map);
		attsM.put(src, new Triple<>(v, en1, t));
		isoToUser = new Mapping<>(isoToUser.ens.map, attsM, isoToUser.fks.map, schemaStr, schemaStr2, checkJava);
		Map<Att, Triple<Var, En, Term<Ty, Sym, En, Fk, Att, Void, Void>>> attsM2 = new HashMap<>(isoFromUser.atts.map);
		attsM2.remove(src);
		isoFromUser = new Mapping<>(isoFromUser.ens.map, attsM2, isoFromUser.fks.map,schemaStr2, schemaStr, checkJava);
	...
	public ColimitSchema<N> wrap(		
			Mapping<Ty, Sym, En, Fk, Att, En, Fk, Att> mapping, 
			Mapping<Ty, Sym, En, Fk, Att, En, Fk, Att> mapping2) {
		if (!mapping.src.equals(schemaStr)) {
			throw new RuntimeException("Source of " + mapping + " \n, namely " + mapping.src + "\ndoes not match canonical colimit, namely " + schemaStr);
		}
		checkIso(mapping, mapping2);
		Ctx<N, Mapping<Ty,Sym,En,Fk,Att,En,Fk,Att>> newMapping = mappingsStr.map((k,v) -> new Pair<>(k, Mapping.compose(v, mapping)));
		return new ColimitSchema<>(ty, nodes, mapping.dst, newMapping);
	}
	
	/*public Mapping<Ty, Sym, En,Fk,Att,String,String,String> getMapping(N n) {
		return Mapping.compose(mappingsStr.get(n), isoToUser);
	}*/

	private ColimitSchema(TypeSide<Ty, Sym> ty, Ctx<N, Schema<Ty, Sym, En, Fk, Att>> nodes, /*Schema<Ty, Sym, Set<Pair<N, En>>, Pair<N, Fk>, Pair<N, Att>> schema, Ctx<N, Mapping<Ty, Sym, En, Fk, Att, Set<Pair<N, En>>, Pair<N, Fk>, Pair<N, Att>>> mappings,*/ Schema<Ty, Sym, En, Fk, Att> schemaStr,
			Ctx<N, Mapping<Ty, Sym, En, Fk, Att, En, Fk, Att>> mappingsStr) {
		super();
		this.ty = ty;
		this.nodes = nodes;
	...
	private static <N> Term<Ty, Sym, En, Fk, Att, Void, Void> 
	conv3(Collage<Ty, Sym, Set<Pair<N, En>>, Pair<N, Fk>, Pair<N, Att>, Void, Void>  col, Term<Ty, Sym, Set<Pair<N,En>>, Pair<N,Fk>, Pair<N,Att>, Void, Void> t) {
	...
	public ColimitSchema(TypeSide<Ty, Sym> ty, Ctx<N, Schema<Ty, Sym, En, Fk, Att>> nodes, 
		...
		for (N n : nodes.keySet()) {
			Schema<Ty, Sym, En, Fk, Att> s = nodes.get(n);
			for (En en : s.ens) {
		...
		Collage<Ty, Sym, Set<Pair<N,En>>, Pair<N,Fk>, Pair<N,Att>, Void, Void> col = new Collage<>(ty.collage()); 
		Ctx<Pair<N, En>, Set<Pair<N, En>>> eqcs = new Ctx<>(uf.toMap());
		col.ens.addAll(eqcs.values());
		Set<Triple<Pair<Var, Set<Pair<N, En>>>, Term<Ty, Sym, Set<Pair<N, En>>, Pair<N, Fk>, Pair<N, Att>, Void, Void>, Term<Ty, Sym, Set<Pair<N, En>>, Pair<N, Fk>, Pair<N, Att>, Void, Void>>> eqs = new HashSet<>();		
	
		
		makeCoprodSchema(col, eqs, eqcs);
	
		DP<Ty, Sym, Set<Pair<N, En>>, Pair<N, Fk>, Pair<N, Att>, Void, Void> dp = AqlProver.create(options, col, ty.js);
		...
		Schema<Ty, Sym, Set<Pair<N,En>>, Pair<N,Fk>, Pair<N,Att>> schema = new Schema<>(ty, col.ens, col.atts.map, col.fks.map, eqs, dp, b);
		
		Pair<Schema<Ty, Sym, En, Fk, Att>, Ctx<N, Mapping<Ty, Sym, En, Fk, Att, En, Fk, Att>>> 
		x = initialUser(options, col, eqs, eqcs, schema);
		
		Schema<Ty, Sym, En, Fk, Att>
		q = quotient(x.first, eqTerms, options);
		
		schemaStr = (Schema<Ty, Sym, En, Fk, Att>) q;
		mappingsStr = new Ctx<>();
		for (N n : x.second.keySet()) {
			Mapping<Ty, Sym, En, Fk, Att, En, Fk, Att> f = x.second.get(n);
	...
	private static Schema<Ty, Sym, En, Fk, Att> quotient(Schema<Ty, Sym, En, Fk, Att> sch, Set<Quad<String, String, RawTerm, RawTerm>> eqTerms, AqlOptions options) {
		Collage<Ty, Sym, En, Fk, Att, Void, Void> col = new Collage<>(sch.collage()); 
		Set<Triple<Pair<Var, En>, Term<Ty, Sym, En, Fk, Att, Void, Void>, Term<Ty, Sym, En, Fk, Att, Void, Void>>> 
		...
		boolean b = ! (Boolean) options.getOrDefault(AqlOption.allow_java_eqs_unsafe);
		DP<Ty, Sym, En, Fk, Att, Void, Void> dp = AqlProver.create(options, col, sch.typeSide.js);
		Schema<Ty, Sym, En, Fk, Att> ret = new Schema<>(sch.typeSide, col.ens, col.atts.map, col.fks.map, eqs0, dp, b);
		return ret;
	}


	public <E> ColimitSchema(DMG<N, E> shape, TypeSide<Ty, Sym> ty, Ctx<N, Schema<Ty, Sym, En, Fk, Att>> nodes, Ctx<E, Mapping<Ty, Sym, En, Fk, Att, En, Fk, Att>> edges, AqlOptions options) {
		this.ty = ty;
		this.nodes = nodes;
		...
		for (E e : shape.edges.keySet()) {
			Mapping<Ty, Sym, En, Fk, Att, En, Fk, Att> s = edges.get(e);
			for (En en : s.src.ens) {
		...
		for (E e : shape.edges.keySet()) {
			...
			for (Fk fk : s.src.fks.keySet()) {
				...
				Term<Ty, Sym, Set<Pair<N, En>>, Pair<N, Fk>, Pair<N, Att>, Void, Void> 
			...
			for (Att att : s.src.atts.keySet()) {
				Triple<Var, En, Term<Ty, Sym, En, Fk, Att, Void, Void>> fk2 = s.atts.get(att);
	...
	private Pair<Schema<Ty, Sym, En, Fk, Att>, Ctx<N, Mapping<Ty, Sym, En, Fk, Att, En, Fk, Att>>> initialUser(AqlOptions options, Collage<Ty, Sym, Set<Pair<N, En>>, Pair<N, Fk>, Pair<N, Att>, Void, Void> col,
			Set<Triple<Pair<Var, Set<Pair<N, En>>>, Term<Ty, Sym, Set<Pair<N, En>>, Pair<N, Fk>, Pair<N, Att>, Void, Void>, Term<Ty, Sym, Set<Pair<N, En>>, Pair<N, Fk>, Pair<N, Att>, Void, Void>>> eqs, Ctx<Pair<N, En>, Set<Pair<N, En>>> eqcs, Schema<Ty, Sym, Set<Pair<N,En>>, Pair<N,Fk>, Pair<N,Att>> schema) {
		Ctx<N, Mapping<Ty, Sym, En,Fk,Att,Set<Pair<N,En>>,Pair<N,Fk>,Pair<N,Att>>> mappings = new Ctx<>();
		
		for (N n : nodes.keySet()) {
			Map<Att, Triple<Var, Set<Pair<N, En>>, Term<Ty, Sym, Set<Pair<N, En>>, Pair<N, Fk>, Pair<N, Att>, Void, Void>>> atts = new HashMap<>();
			...
			Mapping<Ty, Sym, En,Fk,Att,Set<Pair<N,En>>,Pair<N,Fk>,Pair<N,Att>> m = new Mapping<>(ens0 , atts, fks, nodes.get(n), schema, false); //TODO aql allow as option?
			mappings.put(n, m);
		}
		
		Collage<Ty, Sym, En, Fk, Att, Void, Void> colX = new Collage<>(ty.collage()); 
		...
		DP<Ty, Sym, En, Fk, Att, Void, Void> dpX = AqlProver.create(options, colX, ty.js);
	
		Schema<Ty, Sym, En, Fk, Att> schemaStr = new Schema<>(ty, colX.ens, colX.atts.map, colX.fks.map, eqsX, dpX, false);
		
		Ctx<N, Mapping<Ty, Sym, En,Fk,Att,En,Fk,Att>> mappingsStr = new Ctx<>();
	...
	private void makeCoprodSchema(Collage<Ty, Sym, Set<Pair<N, En>>, Pair<N, Fk>, Pair<N, Att>, Void, Void> col, Set<Triple<Pair<Var, Set<Pair<N, En>>>, Term<Ty, Sym, Set<Pair<N, En>>, Pair<N, Fk>, Pair<N, Att>, Void, Void>, Term<Ty, Sym, Set<Pair<N, En>>, Pair<N, Fk>, Pair<N, Att>, Void, Void>>> eqs,
		...
		for (N n : nodes.keySet()) {
			...
			for (Triple<Pair<Var, En>, Term<Ty, Sym, En, Fk, Att, Void, Void>, Term<Ty, Sym, En, Fk, Att, Void, Void>> eq : s.eqs) {
				Term<Ty, Sym, Set<Pair<N, En>>, Pair<N, Fk>, Pair<N, Att>, Void, Void> 
	...
	private void checkIso(Mapping<Ty, Sym, En, Fk, Att, En, Fk, Att> F, Mapping<Ty, Sym, En, Fk, Att, En, Fk, Att> G) {
		isoOneWay(F, G, " when composing (toUser ; fromUser)");
		isoOneWay(G, F, " when composing (fromUser ; toUser)");
	}

	private void isoOneWay(Mapping<Ty, Sym, En, Fk, Att, En, Fk, Att> F, Mapping<Ty, Sym, En, Fk, Att, En, Fk, Att> G, String str) {
		if (!F.dst.equals(G.src)) {
			throw new RuntimeException("Target of " + F + " \n, namely " + F.dst + "\ndoes not match source of " + G + ", namely " + F.src + "\n" + str);
		}
		Mapping<Ty, Sym, En, Fk, Att, En, Fk, Att> f = Mapping.compose(F, G);
		for (En en : f.src.ens) {
			En en2 = f.ens.get(en);
			if (!en.equals(en2)) {
		...
		for (Fk fk : f.src.fks.keySet()) {
			...
			Var v = new Var("v");
			Term<Ty, Sym, En, Fk, Att, Void, Void> t = Term.Fks(fk2.second, Term.Var(v));
			Term<Ty, Sym, En, Fk, Att, Void, Void> s = Term.Fk(fk, Term.Var(v));
		...
		for (Att att : f.src.atts.keySet()) {
			Triple<Var, En, Term<Ty, Sym, En, Fk, Att, Void, Void>> att2 = f.atts.get(att);
			Var v = att2.first;
			Term<Ty, Sym, En, Fk, Att, Void, Void> t = att2.third; //Term.Fks(att2.second, Term.Var(v));
			Term<Ty, Sym, En, Fk, Att, Void, Void> s = Term.Att(att, Term.Var(v));
	...
	private Mapping<Ty, Sym, En, Fk, Att, En, Fk, Att> conv5(Collage<Ty, Sym, Set<Pair<N, En>>, Pair<N, Fk>, Pair<N, Att>, Void, Void> col, Schema<Ty, Sym, En, Fk, Att> schemaStr, Mapping<Ty, Sym, En, Fk, Att, Set<Pair<N, En>>, Pair<N, Fk>, Pair<N, Att>> m) {
		Map<En, En> ens = m.ens.map((k,v) -> new Pair<>(k, conv1(v))).map;
		Map<Att, Triple<Var, En, Term<Ty, Sym, En, Fk, Att, Void, Void>>> 
